...
#error "Emsdk not  Support PTHREADS!!!"
#endif
...
LogLevel logLevel = kLogLevel_Core;//kLogLevel_None;
...
int openCodecContext(AVFormatContext *fmtCtx, enum AVMediaType type, int *streamIdx, AVCodecContext **decCtx) {
    ...
    do {
        ...
        *decCtx = avcodec_alloc_context3(dec);
        ...
        simpleLog("simple thread cnt = %d.", (*decCtx)->thread_count);
...
ErrorCode decodePacket(AVPacket *pkt, int *decodedLen) {
    ...
    *decodedLen = 0;
   // simpleLog("%s: stream idx = %d, pktlen = %d\n", __FUNCTION__, pkt->stream_index, pkt->size);
   ...
   // simpleLog("%s: avcodec_send_packet ok\n", __FUNCTION__);
    while (ret >= 0) {
...
int writeToFile(unsigned char *buff, int size) {
    ...
    do {
        ...
        ret = canWriteBytes;
        simpleLog("file size : %lld, pos = %lld\n", decoder->fileSize, decoder->fileWritePos);
    } while (0);
    return ret;
}
...
ErrorCode openDecoder(int *paramArray, int paramCount, long videoCallback, long audioCallback, long requestCallback) {
    ...
    //    av_register_all();
     //   avcodec_register_all();
        ...
        AVInputFormat *infmt = NULL;//av_find_input_format("avs3");
        simpleLog("+++infmt = %p\n", infmt);
        r = avformat_open_input(&decoder->avformatContext, NULL, infmt, NULL);
        if (r != 0) {
            ret = kErrorCode_FFmpeg_Error;
            char err_info[32] = { 0 };
      ...
      //  simpleLog("1: avio tell: %llu; size: %llu\n", avio_tell(ioContext), avio_size(ioContext));
    ...
    //    simpleLog("2: avio tell: %llu; size: %llu\n", avio_tell(ioContext), avio_size(ioContext));
   ...
   //     av_seek_frame(decoder->avformatContext, -1, 0, AVSEEK_FLAG_BACKWARD);
        simpleLog("3: avio tell: %llu; size: %llu\n", avio_tell(ioContext), avio_size(ioContext));
...
ErrorCode decodeOnePacket() {
   ...
   // simpleLog("+++decodeOnePacket");
    do {
        ...
        AVIOContext  *ioContext = decoder->avformatContext->pb;
       // simpleLog("4: avio tell: %llu; size: %llu\n", avio_tell(ioContext), avio_size(ioContext));
        ...
        if (r == AVERROR_EOF) {
             ...
             simpleLog("5: avio tell: %llu; size: %llu\n", avio_tell(ioContext), avio_size(ioContext));
            ret = kErrorCode_Eof;
            break;
        }
     //   simpleLog(" +++ r = %d\n", r);
